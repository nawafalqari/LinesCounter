def only(file, file_types:list=[]):
    file_types = tuple(file_types)
    if not file.name.endswith(file_types):
        return (0, 0, 0)
...
def handle_only(files:list, file_types:list=[]):
    file_lines = 0
    file_words = 0
    file_chars = 0
    ...
    for file in files:
        if (not isinstance(file, list)) and not file.name.endswith(t_file_types):
            continue
        elif isinstance(file, list):
            files_full_data = handle_only(file, t_file_types)
            file_lines += files_full_data[0]
            file_words += files_full_data[1]
            file_chars += files_full_data[2]
        else:
            file_full_data = only(file)
